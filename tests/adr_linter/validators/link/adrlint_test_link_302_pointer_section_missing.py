# -*- coding: utf-8 -*-
# !/usr/bin/env python3
# tests/adr_linter/validators/link/adrlint_test_link_302_pointer_section_missing.py
...
def test_adrlint_link302_ptr_to_missing_section_emits_warning(
    _route_and_reset_workspace,
):
    """
    Base ADR lacks 'glossary' section; delta uses ptr to 'glossary'
    → MUST emit ADR-LINK-302.
    """
...
def test_adrlint_link302_ptr_to_existing_section_is_ok(
    _route_and_reset_workspace,
):
    """
    Base ADR includes 'glossary' section; delta uses ptr to 'glossary'
    → should NOT emit ADR-LINK-302.
    """
...
def test_adrlint_link302_ptr_to_normative_section_ignored(
    _route_and_reset_workspace,
):
    """
    Delta uses ptr to missing normative section (decision_details)
    → should NOT emit ADR-LINK-302 (normative keys handled by ADR-LINK-304).
    """
...
```"""
    ...
    # Should not emit LINK-302 for normative sections
    assert not _has_code(rpt, _ERROR_CODE)
