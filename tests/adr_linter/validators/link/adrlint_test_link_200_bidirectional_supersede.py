# -*- coding: utf-8 -*-
# !/usr/bin/env python3
# tests/adr_linter/validators/link/adrlint_test_link_200_bidirectional_supersede.py
...
Covers:
  1) Missing reciprocal (should FAIL with ADR-LINK-200)
...
def _write_pair_for_missing_recip(
    ...
    """
    base_path = f"docs/adr-new/{base_id}-base.md"
    new_path = f"docs/adr-new/{new_id}-new.md"
...
def _write_triplet_one_missing(
    ...
    Returns path to B.
    """
    a_path = f"docs/adr-new/{a_id}-old.md"
    b_path = f"docs/adr-new/{b_id}-new.md"
    c_path = f"docs/adr-new/{c_id}-old.md"
...
def test_adrlint200_missing_reciprocal_superseded_by_emits_error(
    ...
    rpt = Report()
    run_all(ctx, rpt)
    assert _has_code(rpt, "ADR-LINK-200")


def test_adrlint200_reciprocal_present_passes(_route_and_reset_workspace):
    ...
    rpt = Report()
    run_all(ctx, rpt)
    assert not _has_code(rpt, "ADR-LINK-200")


def test_adrlint200_multiple_supersedes_one_missing_reciprocal_emits_error(
    _route_and_reset_workspace,
):
    """
