# -*- coding: utf-8 -*-
# !/usr/bin/env python3
# tests/adr_linter/registry/adrlint_test_registry_003_registry_tests_parity.py

"""
ADRLINT-meta · ADR-registry ↔ tests parity (per-file validators)

Checks that for every code in ORDERED_RULES_PER_FILE there is a corresponding
test file under tests/adrlinter/validators/<band>/ with a filename beginning:
    adrlint_test_<band>_<NNN>*.py

Examples that pass:
  - tests/adrlinter/validators/schema/adrlint_test_schema_003_keys_order.py
  - tests/adrlinter/validators/link/
       adrlint_test_link_222_fork_no_rationale_unit.py

Behavior:
  - Default: non-strict. If any are missing, the test XFAILs with a clear
    summary so your suite stays green during migrations.
  - Strict mode: set ADR_TEST_PARITY_STRICT=1 to make missing items FAIL.
...
from adr_linter.validators.registry import ORDERED_RULES_PER_FILE

# Known intentional gaps (keep small!).
# TOREVEIW: Before refactoring, ADR-LINK-202 was the only identified (known?)
...
BAND_DIR = {
    ...
    "META": "meta",
    "NORM": "norm",
    "TEMPLATE": "template",  # directory is 'template', filenames use 'templt'
}
...
def _tests_dir_for_band(band: str) -> Path:
    return _tests_root() / "adr_linter" / "validators" / BAND_DIR[band]
...
def _pattern_for(band: str, num: str) -> str:
    ...
    """
    # TOREVIEW: If the code works, delete the next section of code
    # token = BAND_DIR[band]
    # keep filename token exactly as used in your test files
    # if band == "TEMPLT":
    #     token = "templt"
    # return f"adrlint_test_{token}_{num}*.py"

    return f"adrlint_test_{band.lower()}_{num}*.py"


def test_adrlint1001_registry001_validation_tests_parity():
    """
    ...
    for code, _fn in ORDERED_RULES_PER_FILE:
        ...
        if band not in BAND_DIR:
            continue

        tests_dir = _tests_dir_for_band(band)
